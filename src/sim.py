...
class sim:
  ...
  def run(this_sim, counter):
    ...
    if(this_sim['exyz'] == None):
      g.print('No exyz file - check path if this was expected.', fhr)
    if(this_sim['neutron'] == None):
      g.print('No neutron data - check input file if this was expected.', fhr)
    if(this_sim['exyz'] != None):
    ...
    ##############################################
      ...
      ncount = this_sim['neutron']['count']
      neutron_energy = []
      if(this_sim['neutron']['dist'] == 'mono'):
        for n in range(ncount):
          neutron_energy.append(this_sim['neutron']['energy'])
      else:
        if(this_sim['neutron']['dist'] == 'flat'):
          energy.set(dist='flat', min=this_sim['neutron']['min'], max=this_sim['neutron']['max'])
        elif(this_sim['neutron']['dist'] == 'maxwell'):
          energy.set(dist='maxwell', mean=this_sim['neutron']['mean'])
        for n in range(ncount):
    ...
    if(procede == False):
      g.print('No exyz or neutron data - ending sim.', fhr)
      return False
    ...
    rr_data_dir = data_dir + "/rr"
    std.make_dir(rr_data_dir)

    fh = open(rr_data_dir + "/reaction_rates.txt", "w")
    fh.write("#################################################################\n")
    ...
    fh = open(rr_data_dir + "/reaction_rates.csv", "w")   
    for target in residual_rrs.keys():
      if(target in particle_rrs.keys()):
        for particle in particle_rrs[target].keys():
          thestr = std.pad_right(particle + ":", 20) + str(particle_rrs[target][particle])
          fh.write(str(target) + "," + str(particle) + "," + str(particle_rrs[target][particle]) + "\n")
      for residual in residual_rrs[target].keys():
        thestr = std.pad_right(particle + ":", 20) + str(particle_rrs[target][particle])
        fh.write(str(target) + "," + str(residual) + "," + str(residual_rrs[target][residual]) + "\n")
    fh.close()
    ...
    for i in range(steps_eob):
      ...
      for target in residual_rrs.keys():
        for residual in residual_rrs[target].keys():
          if(residual not in eob_idata[target].keys()):
          ...
          if(not isotopes.is_stable(residual)):
            ...
            if(i == (steps_eob-1)):
              for k in r['tally'].keys():
                ...
                eob_idata[target][residual][k]['n0'] = r['tally'][k]['nend']



    # End of beam activity    
    #for i in range(steps_eos-1):       
    for i in range(tc+1, td):
      time = t[i] - beam_duration
      sim.activity_log.write("##################################\n")
      sim.activity_log.write("[" + str(i) + "]   ")
      sim.activity_log.write(str(t[i]) + "    (" + str(time) + ")\n")
      sim.activity_log.write("##################################\n")
      sim.activity_log.write("\n")
      for target in residual_rrs.keys():
        for residual in residual_rrs[target].keys():
          if(not isotopes.is_stable(residual)):
            ...
            for k in r['tally'].keys():
              ...
              sim.activity_log.write(str(results['n_tree'][target][k][i]))
              sim.activity_log.write("\n")
              #print(target, k, r['tally'][k]['n0'], r['tally'][k]['nend'])
    ...
    for target in results['n_tree'].keys():
      for residual in results['n_tree'][target].keys():
        eob_idata[target][residual] = {}
        eob_idata[target][residual]['w'] = 0.0
        eob_idata[target][residual]['n0'] = results['n_tree'][target][residual][steps_eob-1]
        print(target, residual, eob_idata[target][residual]['n0'])
    print()
     
    pprint.pprint(eob_idata)

    
    for target in residual_rrs.keys():
      for residual in residual_rrs[target].keys():
        if(not isotopes.is_stable(residual)):
          print(target, residual, "stable")
        else:
          print(target, residual, "")
    print()
    idata = {}
    idata[14026] = {'w': 0.0, 'n0': 9720.144760880785}
    r = decay.calculate(14026, 60.0, idata)
    pprint.pprint(r)
    exit()
    # End of beam activity    
    #for i in range(steps_eos-1):       
    for i in range(1): 
      ...
      time = 0.1
      sim.activity_log.write("##################################\n")
      sim.activity_log.write("[" + str(i + steps_eob) + "]   ")
      sim.activity_log.write(str(time) + "   (" + str(t[i+steps_eob]) + ")\n")
      sim.activity_log.write("##################################\n")
      sim.activity_log.write("\n")
      print("Time: ", time)
      for target in residual_rrs.keys():
...
Dose 10W+ gammas absorbed for 1s mostly fatal (1000+ Rads)
    ...
    # Projectile
    this_plot_dir = plot_dir + "/projectile"
    std.make_dir(this_plot_dir)

    if(this_sim['neutron'] != None):
      plt.hist(neutron_energy, density=True, bins=25)
      plt.savefig(this_plot_dir + '/neutron_energy.eps', format='eps', transparent=False)
      plt.close('all') 


    # XS Plots
    this_plot_dir = plot_dir + "/xs"
    for target in residual_rrs.keys():
      if(target is not None):
        talys.plot_rxs(this_plot_dir, projectile, target)
        talys.plot_pxs(this_plot_dir, projectile, target)
